...
namespace FairyGUI
{
    ...
    public class RegularPolygonMesh : IMeshFactory, IHitTest
    {
        ...
        public void OnPopulateMesh(VertexBuffer vb)
        {
            ...
            vb.AddVert(new Vector3(centerX, centerY, 0),
                centerColor == null ? color : (Color32)centerColor);
            for (int i = 0; i < sides; i++)
            {
                float r = radius;
                if (distances != null) { r *= distances[i]; }
                float sine = MathF.Sin(angle), cosine = MathF.Cos(angle);
                float xv = cosine * (r - lineWidth);
                float yv = sine * (r - lineWidth);
