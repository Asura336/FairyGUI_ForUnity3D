...
namespace FairyGUI
{
    ...
    public class LineMesh : IMeshFactory
    {
        ...
        static readonly List<Vector3> points = new List<Vector3>(64);
        static readonly List<float> ts = new List<float>(64);
        ...
        public void OnPopulateMesh(VertexBuffer vb)
        {
            ...
            for (int si = 0; si < segCount; si++)
            {
                ...
                for (int i = 1; i < cnt; i++)
                {
                    ...
                    Vector3 widthVector = Vector3.Cross(lineVector, Vector3.forward);
                    ...
                    if (i == 1)
                    {
                        float u = repeatFill
                            ? tc * uvRatio * uvMax.x
                            : Mathf.Lerp(uvMin.x, uvMax.x, t + ratio * ts[i - 1]);

                        var _widthVector = widthVector * (lw * 0.5f);
                        vb.AddVert(p0 - _widthVector, c0, new Vector2(u, uvMax.y));
                        vb.AddVert(p0 + _widthVector, c0, new Vector2(u, uvMin.y));
                    ...
                    {
                        float u = repeatFill ? tc * uvRatio * uvMax.x : Mathf.Lerp(uvMin.x, uvMax.x, tc);
                        var _widthVector = widthVector * (lw * 0.5f);
                        vb.AddVert(p1 - _widthVector, c1, new Vector2(u, uvMax.y));
                        vb.AddVert(p1 + _widthVector, c1, new Vector2(u, uvMin.y));
                    }
        ...
        {
            Vector2 widthVector = Vector3.Cross(p0 - p1, Vector3.forward);
            widthVector.Normalize();
            float lw_half = lw * 0.5f;
            widthVector *= lw_half;
            Vector2 lineVector = (p0 - p1).normalized * lw_half;

            int sides = Math.Max(Mathf.CeilToInt(ToolSet.PI_HALF * lw), 6);
