...
namespace FairyGUI
{
    ...
    public class Container : DisplayObject
    {
        ...
        {
            get { return _mask; }
            [Obsolete("增加 SDF 图元后设置遮罩的过程需要判断遮罩的图元类型，用 SetMask 替代")]
            set
            {
                if (_mask != value)
                {
        ...
        /// 设置遮罩，附带检查
        /// </summary>
        /// <param name="maskObject"></param>
        public void SetMask(GObject maskObject)
        {
            if (maskObject is not (GGraph or GImage))
            {
                ToolSet.GetFriendlyUrl(gOwner, out var pkgName, out var itemName);
                throw new ArgumentException($"仅能使用图片或者图形作为遮罩，在 {(gOwner?.name ?? "undefined")}({(pkgName ?? "UnknownPackage")}/{(itemName ?? "UnknownItem")})");
            }
            if (maskObject is GGraph graph && graph.shape.isSDFShape)
            {
                if (Debug.isDebugBuild && ToolSet.GetFriendlyUrl(gOwner, out var pkgName, out var itemName))
                {
                    Debug.LogWarning($"在 {(gOwner?.name ?? "undefined")}({(pkgName ?? "UnknownPackage")}/{(itemName ?? "UnknownItem")}) 设置遮罩的图形 {maskObject.name} 是 SDF 图形，可能无法正确处理剪裁，将退化为普通图形。要修复此条警告，为遮罩图形的名字加上 nosdf- 前缀");
                }
                graph.FallbackSDF();
            }

            var value = maskObject?.displayObject;
            // 确保到这一步使用遮罩的图形由网格模拟形状
            // original...
            if (_mask != value)
            {
                _mask = value;
        ...
        override protected DisplayObject HitTest()
        {
            ...
            var localScale = this.cachedTransform.localScale;
