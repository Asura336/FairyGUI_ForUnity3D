    ...
    public class DisplayObject : EventDispatcher, IBatchable
    {
        ...
        void UpdateTransformMatrix()
        {
            ...
            if (_perspective)
            {
                ...
                Matrix4x4 o = default;
                matrix.Mul(Matrix4x4.TRS(default, Quaternion.Euler(_rotation), Vector3.one), ref o);
                matrix = o;
            }
        ...
        void UpdatePivotOffset()
        {
            ...
            matrix.MultiplyPoint(new Vector3(px, -py, 0), ref _pivotOffset);
        ...
        virtual public Rect GetBounds(DisplayObject targetSpace)
        {
            ...
            else if (targetSpace == parent && _rotation.z == 0)
            {
                ...
                return new Rect(localPosition.x, -localPosition.y,
                    _contentRect.width * localScale.x, _contentRect.height * localScale.y);
            }
            else
            {
        ...
        public Vector3 WorldToLocal(Vector3 worldPoint, Vector3 direction)
        {
            ...
            if (localPoint.z != 0) //如果对象绕x轴或y轴旋转过，或者对象是在透视相机，那么z值可能不为0，
            {
                ...
                float distOnLine = (Vector3.zero - localPoint).Dot(Vector3.forward) / direction.Dot(Vector3.forward);
                if (float.IsInfinity(distOnLine))
                {
                    return Vector2.zero;
                }

                localPoint += direction * distOnLine;
            }
            else if (_vertexMatrix != null)
            {
                ...
                ref var _vMatrix = ref _vertexMatrix.matrix;

                //center -= _vMatrix.MultiplyPoint(center);
                Vector3 cp = default;
                _vMatrix.MultiplyPoint(center, ref cp);
                center.Minus3(cp);

                Matrix4x4 mm = _vMatrix.inverse;

                //localPoint -= center;
                localPoint.Minus3(center);
                localPoint = mm.MultiplyPoint(localPoint);

                //Vector3 camPos = mm.MultiplyPoint(_vertexMatrix.cameraPos);
                Vector3 camPos = default;
                mm.MultiplyPoint(_vertexMatrix.cameraPos, ref camPos);
        ...
        public Vector3 LocalToWorld(Vector3 localPoint)
        {
            ...
            if (_vertexMatrix != null)
            {
                ...
                //localPoint += center;
                Vector3 lp = default;
                _vMatrix.MultiplyPoint(localPoint, ref lp);
                localPoint = lp;
                localPoint.Plus3(center);
        ...
        /// <returns></returns>
        public unsafe Vector2 TransformPoint(Vector2 point, DisplayObject targetSpace)
        {
            if (targetSpace == this)
            {
                return point;
            ...
            p3.x = point.x;
            p3.y = point.y;
            var worldPoint = LocalToWorld(p3);
            if (targetSpace != null)
            {
                worldPoint = targetSpace.WorldToLocal(worldPoint, Vector3.back);
            }
        ...
        public Rect TransformRect(Rect rect, DisplayObject targetSpace)
        {
            ...
            else
            {
                ...
                TransformRectPoint(xMin, yMin, targetSpace, ref vec4);
                TransformRectPoint(xMax, yMin, targetSpace, ref vec4);
                TransformRectPoint(xMin, yMax, targetSpace, ref vec4);
                TransformRectPoint(xMax, yMax, targetSpace, ref vec4);
        ...
        virtual public void Update(UpdateContext context)
        {
            if (_checkPixelPerfect != 0)
            {
                ...
                {
                    Vector3 _lp = cachedTransform.localPosition;
                    Vector3 v = _lp;
                    v.x = Mathf.Round(v.x);
                    v.y = Mathf.Round(v.y);
                    _pixelPerfectAdjustment = v - _lp;
