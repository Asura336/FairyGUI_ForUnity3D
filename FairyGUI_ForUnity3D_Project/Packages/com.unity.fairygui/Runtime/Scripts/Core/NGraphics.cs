...
namespace FairyGUI
{
    ...
    public class NGraphics : IMeshFactory, IBatchable
    {
        ...
        public event Action<NGraphics> sdf_onBeforePopulateMesh;
        public event Action<NGraphics, float> sdf_onChangeAlpha;
        ...
        public NTexture texture
        {
            ...
            set
            {
                if (_texture != value)
                {
                    ...
                    _texture = value;
                    if (_customMatarial != 0 && _materialAllocated)
                    {
        ...
        public Material material
        {
            get
            {
                if (_customMatarial == 0 && _manager != null && !_materialAllocated)
                {
                    _material = _manager.GetMaterial(_materialFlags, blendMode, 0);
                    _materialAllocated = true;
                }
            ...
            set
            {
                if ((_customMatarial & 128) != 0 && _materialAllocated)
                {
                    Object.DestroyImmediate(_material);
                    _material = null;
                    _materialAllocated = false;
                }

                _material = value;
                _materialAllocated = _material != null;
                if (_materialAllocated)
                {
                    _customMatarial = 1;
        ...
        /// <summary>
        /// 会在 TextMeshPro 字符和图片的颜色效果的地方使用，
        ///如果用于 URP，材质属性块会造成性能负担，但现在 FGUI 缺省的做法是多图元共享材质
        /// </summary>
        /// <returns></returns>
        ...
        public void Dispose()
        {
            ...
            _stencilEraser = null;
            meshModifier = null;
            sdf_onBeforePopulateMesh = null;
            sdf_onChangeAlpha = null;
            if (subInstances != null)
            {
                foreach (var sub in subInstances)
                {
        ...
        public void Update(UpdateContext context, float alpha, bool grayed)
        {
            ...
            // for SDF
            sdf_onChangeAlpha?.Invoke(this, _alpha);
            ...
            if (_customMatarial != 0)
            {
                if ((_customMatarial & 2) != 0 && _materialAllocated)
                {
        ...
        void UpdateMeshNow()
        {
            ...
            sdf_onBeforePopulateMesh?.Invoke(this);  // for SDF
