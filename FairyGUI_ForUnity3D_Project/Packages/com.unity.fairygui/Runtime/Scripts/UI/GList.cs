...
namespace FairyGUI
{
    ...
    public class GList : GComponent
    {
            ...
            : base()
        {
            ...
            _pool = new GObjectPool(container.cachedTransform, 64);
        ...
        void RefreshVirtualList(object param)
        {
            ...
            // 在调用 itemRenderer 之前记录预计算的滚动条可见性
            if (scrollPane != null)
            {
                horizontalScrollbarVisible = scrollPane.hzScrollBar != null
                    && (!scrollPane._displayInDemand || cw > viewWidth);
                verticalScrollbarVisible = scrollPane.vtScrollBar != null
                    && (!scrollPane._displayInDemand || ch > viewHeight);
            }
            else
            {
        ...
        bool HandleScroll1(bool forceUpdate)
        {
            ...
            while (curIndex < _realNumItems && (end || curY < max))
            {
                ...
                if (needRender)
                {
                    ...
                    {
                        _sizeHandled = true;
                        ii.obj.SetSize(partSize + scrollPane.vtScrollBar.width, ii.obj.height, true);
                    }
                    if (!_sizeHandled
                        && _autoResizeItem
                        && (_layout == ListLayoutType.SingleColumn || _columnCount > 0))
                    {
                        _sizeHandled = true;
                        ii.obj.SetSize(partSize, ii.obj.height, true);
                    }
                    ...
                    {
                        deltaSize += Mathf.CeilToInt(ii.obj.height) - ii.size.y;
                        if (curIndex == newFirstIndex && oldFirstIndex > newFirstIndex)
                        {
                            //当内容向下滚动时，如果新出现的项目大小发生变化，需要做一个位置补偿，才不会导致滚动跳动
                            firstItemDeltaSize = Mathf.CeilToInt(ii.obj.height) - ii.size.y;
                        }
                    }
                    ii.size.x = Mathf.CeilToInt(ii.obj.width);
                    ii.size.y = Mathf.CeilToInt(ii.obj.height);
                }
        ...
        bool HandleScroll2(bool forceUpdate)
        {
            ...
            while (curIndex < _realNumItems && (end || curX < max))
            {
                ...
                if (needRender)
                {
                    ...
                    bool _sizeHandled = false;
                    if (!_sizeHandled
                        && _layout == ListLayoutType.SingleRow
                        && _autoResizeItem
                        && scrollPane.hzScrollBar != null
                        && !horizontalScrollbarVisible)
                    {
                        _sizeHandled = true;
                        ii.obj.SetSize(ii.obj.width, partSize + scrollPane.hzScrollBar.height, true);
                    }
                    if (!_sizeHandled
                        && _autoResizeItem
                        && (_layout == ListLayoutType.SingleRow || _lineCount > 0))
                    {
                        _sizeHandled = true;
                        ii.obj.SetSize(ii.obj.width, partSize, true);
                    }
                    ...
                    if (curIndex % _curLineItemCount == 0)
                    {
                        deltaSize += Mathf.CeilToInt(ii.obj.width) - ii.size.x;
                        if (curIndex == newFirstIndex && oldFirstIndex > newFirstIndex)
                        {
                            //当内容向下滚动时，如果新出现的一个项目大小发生变化，需要做一个位置补偿，才不会导致滚动跳动
                            firstItemDeltaSize = Mathf.CeilToInt(ii.obj.width) - ii.size.x;
                        }
                    }
                    ii.size.x = Mathf.CeilToInt(ii.obj.width);
                    ii.size.y = Mathf.CeilToInt(ii.obj.height);
