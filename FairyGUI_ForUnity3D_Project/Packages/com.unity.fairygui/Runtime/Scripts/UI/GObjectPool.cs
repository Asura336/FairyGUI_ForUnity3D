...
namespace FairyGUI
{
    ...
    public class GObjectPool
    {
        ...
        readonly int initQueueCapacity = 4;

        readonly Dictionary<string, Queue<GObject>> _pool;
        readonly Transform _manager;

        /// <summary>
        /// 需要设置一个manager，加入池里的对象都成为这个manager的孩子
        /// </summary>
        /// <param name="manager"></param>
        /// <param name="initPoolCapacity">构造内部队列时设置初始长度</param>
        /// <param name="initPoolChannelCapacity">构造内部字典时设置初始长度</param>
        public GObjectPool(Transform manager, int initPoolCapacity, int initPoolChannelCapacity)
        {
            _manager = manager;
            _pool = new Dictionary<string, Queue<GObject>>(initPoolChannelCapacity);
            initQueueCapacity = initPoolCapacity;
        }
        ...
        /// <param name="manager"></param>
        /// <param name="initPoolCapacity">构造内部队列时设置初始长度</param>
        public GObjectPool(Transform manager, int initPoolCapacity)
        {
            _manager = manager;
        ...
        public void ReturnObject(GObject obj)
        {
            ...
            if (!_pool.TryGetValue(url, out arr))
            {
                arr = new Queue<GObject>(initQueueCapacity);
                _pool.Add(url, arr);
            }
