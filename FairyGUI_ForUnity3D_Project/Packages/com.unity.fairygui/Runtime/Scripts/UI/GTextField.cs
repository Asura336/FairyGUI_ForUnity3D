...
namespace FairyGUI
{
    ...
    public class GTextField : GObject, ITextColorGear
    {
        ...
        protected const int smallTextLength = 4096;
        protected static readonly string smallTextBuffer = new string('\0', smallTextLength);
#endif
        virtual protected void SetTextFieldText()
        {
            ...
            if (_ubbEnabled)
            {
                ...
                if (buffer.Length == 0)
                {
                    _textField.htmlText = string.Empty;
                }
                else
                {
                    ReadOnlySpan<char> sp;
                    XMLUtils.EncodeString(buffer);
                    ...
                    parser.Parse(dst);
                    _textField.htmlText = dst.ToString();
                }
#else
                XMLUtils.EncodeString(buffer);
                _textField.htmlText = UBBParser.inst.Parse(buffer.ToString());
#endif
            }
            else
            {
                _textField.text = buffer.Length == 0 ? string.Empty : buffer.ToString();
            }
        }
        ...
        protected void ParseTemplate(string template, StringBuilder buffer)
        {
            int pos1 = 0, pos2 = 0;
            int pos3;
            string tag;
        ...
        override protected void HandleSizeChanged()
        {
            ...
            else if (_textField.autoSize != AutoSizeType.Both)
            {
                if (_textField.autoSize == AutoSizeType.Height)
                {
                    ...
                    if (!string.IsNullOrEmpty(_text)) //文本为空时，1是本来就不需要调整， 2是为了防止改掉文本为空时的默认高度，造成关联错误
                    {
